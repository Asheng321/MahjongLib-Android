...
public class MahjongPuzzlesFragment extends Fragment implements OnTabChangeListener {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		int defaultTab = PreferenceManager.getDefaultSharedPreferences(getActivity()).getInt(
				getString(R.string.pref_mahjong_difficulty_tab), 0);
		if (defaultTab == 0) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(R.layout.mahjong_puzzles, null);
	}

	public void onTabChanged(String tabId) {
		PreferenceManager.getDefaultSharedPreferences(getActivity()).edit()
				.putInt(getString(R.string.pref_mahjong_difficulty_tab), getTabHost().getCurrentTab()).commit();
		updateTab(tabId);
	}

	private TabHost getTabHost() {
		return (TabHost) getView().findViewById(R.id.mahjong_tabs);
	}

	private void updateTab(String tabId) {
		FragmentManager fm = getFragmentManager();
		if (fm.findFragmentByTag(tabId) == null) {
			Fragment frag = new MahjongTabFragment();
